...
You can also work with a vector of characters in R. A character is anything that is not a number. A character can be created by putting a value in single quotes ''.
...
Let's say you wanted to access the third entry in this column. We can use the same indexing rules introduced earlier.

```{r}
mydata$x[3]
```

If you've worked with indexing blocks of data before, you know that you can often just index both the rows and columns (e.g. mydata[row,column]). This is totally valid but can be a little unclear when trying to read through the code (i.e. what is that column??). 
...
Wait. We never changed y to a factor. What? What just happened? I want those hours of my life back because of this this oversight. You will be there. I promise. The best workaround is to simply specify that you would not like this coercion to happen. 
