...
<img src="dog_balancing.gif" alt="dogbalancing"></img>
...
Here the for loop will go through 100 times (starting at 1 and going until i = 100). I end up using for loops for a number of things. When reading data in from a director, I've used for loops to read in all the 'i' files in from the directory. I have also used it for simulating data sets (e.g. MCMC or bootstrap simulations).

### Note on efficiency
for loops can be great for a lot of things. I think they're best on smaller data sets because one thing they're notably bad with is when you are working with larger datasets. When you're working with larger datasets, you'll want to do something called vectorized operations or use a package that does vectorized operations for you to speed up processing.
